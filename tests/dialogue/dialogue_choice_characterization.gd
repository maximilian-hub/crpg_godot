...
func _test_multi_choice_wrapping_and_targets() -> void:
	...
	controller.set_page_complete(true)
	_check(controller.move(-1) and controller.selected_index == 2, "left navigation wraps first choice to last")
	_check(controller.move(1) and controller.selected_index == 0, "right navigation wraps last choice to first")
	controller.move(1)
