...
func _ready() -> void:
	...
	_check(black_magic.cooldown_presentation.active_mote_count() == black_king.base_cooldown and black_magic.king_aura.particle_power < black_magic.cooldown_presentation.profile.ready_particle_power, "full activation preserves the opening cooldown motes without applying ready-aura treatment")
