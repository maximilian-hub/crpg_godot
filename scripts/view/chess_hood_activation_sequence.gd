...
## Hood uses the same phase/resolve machinery but compresses the invocation
...
func _apply_visual_state() -> void:
	...
	match phase:
		Phase.INVOCATION:
			...
			hand_aura.set_silhouette_power(lerpf(0.0, 0.85, progress * progress))
			hand_aura.set_particle_power(0.0)
		...
		Phase.RESPONSE, Phase.BUILDUP:
			...
			hand_aura.set_silhouette_power(lerpf(0.85, 1.0, progress))
			hand_aura.set_particle_power(lerpf(0.0, 0.25, progress))
			king_aura.set_silhouette_power(lerpf(0.0, 0.65, progress))
			king_aura.set_particle_power(0.0)
			king_aura.set_silhouette_fill(0.0)
		Phase.CLIMAX:
